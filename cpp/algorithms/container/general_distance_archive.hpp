//
// Created by Luca Grillotti on 10/07/2020.
//

#ifndef AURORA_GENERAL_DISTANCE_ARCHIVE_HPP
#define AURORA_GENERAL_DISTANCE_ARCHIVE_HPP

#include <sferes/qd/container/tools.hpp>
...
namespace sferes {
    namespace qd {
        namespace container {
            ...
            template <typename Phen, typename Storage, typename Params, typename DistanceCalculator>
            class GeneralDistanceArchive {
            public:
                ...
                bool add(indiv_t i1)
                {
                    ...
                    if (i1->fit().dead())
                        return false;
                    if (_archive.size() == 0
                        || (DistanceCalculator::getValue(_archive.nearest(p_i1).first, p_i1) > Params::nov::l)) // ADD because new
                    {
                    ...
                    else // archive size >= 2
                    {
                        ...
                        if (DistanceCalculator::getValue(p_i1, neigh_cur[1].first) < (1 - Params::nov::eps) * Params::nov::l) // too close the second NN -- this works better
                        {
                ...
                void compute_novelty_for_specific_population(pop_t& pop) {
                    _archive.optimize();

                    _p_novelty nov(_archive);
                    std::for_each(pop.begin(), pop.end(), nov);
                }
                ...
                static double get_novelty(const indiv_t& indiv, const_knn_iterator_t begin, const_knn_iterator_t end)
                {
                    ...
                    for (const_knn_iterator_t it = begin; it != end; it++) {
                        sum += DistanceCalculator::getValue((*it).first, p_indiv);
                    }
                ...
                bool
                direct_add(const indiv_t& tobeinserted, const bool add_if_already_in_archive = true)
                {
                  point_t p_tobeinserted(tobeinserted->fit().desc().data(),
                                         tobeinserted->fit().desc().size());

                  if (not add_if_already_in_archive) {
                    if (_archive.contains(p_tobeinserted)) {
                      return false;
                    }
                  }
              ...
              protected:
                ...
                struct _p_novelty {
                    ...
                    void operator()(indiv_t& indiv) const
                    {
                        ...
                        auto res = GeneralDistanceArchive::get_nov_and_lq(indiv, _apop);
...
#endif //AURORA_GENERAL_DISTANCE_ARCHIVE_HPP
