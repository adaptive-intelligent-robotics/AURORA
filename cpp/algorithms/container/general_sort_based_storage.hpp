...
#ifndef AURORA_GENERAL_SORT_BASED_STORAGE_HPP
#define AURORA_GENERAL_SORT_BASED_STORAGE_HPP
...
namespace sferes {
    namespace qd {
        namespace container {
            ...
            template <int p>
            struct DistancePNorm {
                double static inline getValue(Eigen::VectorXd p_1, Eigen::VectorXd p_2) {
                    return (p_1 - p_2).lpNorm<p>();
                }
            };
            ...
            template <typename Value, typename DistanceCalculator> class GeneralDistanceSortBasedStorage {
            public:
                ...
                std::vector<data_t> knn(const Eigen::VectorXd& p, int k) const
                {
                    ...
                    for (size_t j = 0; j < _data.size(); ++j) {
                        distances[j] = DistanceCalculator::getValue(p, _data[j].first);
                    }
...
#endif //AURORA_GENERAL_SORT_BASED_STORAGE_HPP
