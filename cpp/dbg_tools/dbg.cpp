//
// Created by Luca Grillotti on 03/09/2020.
//
 ...
 *  - Additionally, the airl::dbg::array_size template utility could be (and was)
...
namespace
{
  ...
  const unsigned int  NUM_DBG_LEVELS   = airl::dbg::all-1;
  ...
  /**
   * Functor to provide comparison of @ref airl::dbg::source_pos structs.
   *
   * @internal
   */
  struct lt_sp
  {
    bool operator()(const airl::dbg::source_pos &a, const airl::dbg::source_pos &b)
    const
    {
      if (a.file == b.file)
      {
   ...
   * constructor magically takes settings from the @ref airl::dbg::default_source.
  ...
  public:
     ...
     * of the boolean.
     */
    void enable(airl::dbg::level lvl, bool enable);

    /**
     * Returns whether or not the level is enabled.
     */
    bool enabled(airl::dbg::level lvl) const
    {
  ...
  private:
     ...
     * of the sources map.
     */
    static unsigned int dbg_source_mask(airl::dbg::level lvl)
    {
      return (lvl != airl::dbg::all) ? 1 << lvl : ALL_SOURCES_MASK;
    }
...
//    typedef map_type::data_type                data_type; // pre 3.3
  ...
  typedef std::map<airl::dbg::source_pos, period_data, lt_sp> period_map_type;
  ...
  airl::dbg::assertion_behaviour behaviour[airl::dbg::all+1] =
    {
      airl::dbg::assertions_abort,
      airl::dbg::assertions_abort,
      airl::dbg::assertions_abort,
      airl::dbg::assertions_abort,
      airl::dbg::assertions_abort,
   ...
   */
  void print_pos(std::ostream &out, const airl::dbg::source_pos &where);

  /**
   * Prints a source_pos to the given ostream in short format
   * (suitable for trace).
   */
  void print_pos_short(std::ostream &out, const airl::dbg::source_pos &where);

  /**
   * Prints period information to the given ostream, if a period has been
   * enabled.
   */
  void print_period_info(std::ostream &out, const airl::dbg::source_pos &where);

  /**
   * Does whatever the assertion_behaviour is set to. If an assertion
   * is triggered, then this will be called.
   */
  void do_assertion_behaviour(airl::dbg::level lvl, constraint_type why,
                              const airl::dbg::source_pos &pos);

  /**
   * Produces a level prefix for the specified level to the
   * given ostream.
   */
  void do_prefix(airl::dbg::level lvl, std::ostream &s);

  /**
   * Used by period_allows below.
   */
  bool period_allows_impl(const airl::dbg::source_pos &where);

  /**
   * Returns whether the period allows the constraint at the specified
   * @ref airl::dbg::source_pos to trigger. This presumes that the assertion
   ...
   */
  inline bool period_allows(const airl::dbg::source_pos &where)
  {
   ...
   */
  void determine_source(airl::dbg::dbg_source &src, const airl::dbg::source_pos &here);
}
...
airl::dbg::dbg_source airl::dbg::default_source = "airl::dbg::private::default_source";
...
void airl::dbg::enable(airl::dbg::level lvl, bool enabled)
{
  out(debug) << prefix(debug) << "airl::dbg::enable(" << LEVEL_NAMES[lvl]
...
void airl::dbg::enable(airl::dbg::level lvl, airl::dbg::dbg_source src, bool enabled)
{
  out(debug) << prefix(debug) << "airl::dbg::enable(" << LEVEL_NAMES[lvl]
             << ",\"" << src << "\","
             << (enabled ? TRUE_STRING : FALSE_STRING) << ")\n";
...
void airl::dbg::enable_all(airl::dbg::level lvl, bool enabled)
{
  out(debug) << prefix(debug) << "airl::dbg::enable_all("
             << LEVEL_NAMES[lvl] << ","
             << (enabled ? TRUE_STRING : FALSE_STRING) << ")\n";
...
std::ostream &airl::dbg::out(airl::dbg::level lvl, airl::dbg::dbg_source src)
{
  std::ostream& s = source_map[src ? src : ""].out(lvl);
  do_prefix(lvl, s);
  return s;
}


void airl::dbg::attach_ostream(airl::dbg::level lvl, std::ostream &o)
{
  out(debug) << prefix(debug) << "airl::dbg::attach_ostream("
...
void airl::dbg::attach_ostream(airl::dbg::level lvl, airl::dbg::dbg_source src, std::ostream &o)
{
  out(debug) << prefix(debug) << "airl::dbg::attach_ostream("
             << LEVEL_NAMES[lvl]
             << ", \"" << src
...
void airl::dbg::detach_ostream(airl::dbg::level lvl, std::ostream &o)
{
  out(debug) << prefix(debug) << "airl::dbg::detach_ostream("
             << LEVEL_NAMES[lvl] << ")\n";
...
void airl::dbg::detach_ostream(airl::dbg::level lvl, airl::dbg::dbg_source src, std::ostream &o)
{
  out(debug) << prefix(debug) << "airl::dbg::detach_ostream("
             << LEVEL_NAMES[lvl]
             << ", \"" << src
...
void airl::dbg::detach_all_ostreams(airl::dbg::level lvl)
{
  out(debug) << prefix(debug) << "airl::dbg::detach_all_ostreams("
             << LEVEL_NAMES[lvl]
             << ")\n";
...
void airl::dbg::detach_all_ostreams(airl::dbg::level lvl, airl::dbg::dbg_source src)
{
  out(debug) << prefix(debug) << "airl::dbg::detach_all_ostreams("
             << LEVEL_NAMES[lvl]
             << ", \"" << src << "\")\n";
...
void airl::dbg::set_prefix(const char *pfx)
{
  out(debug) << prefix(debug) << "airl::dbg::set_prefix(" << pfx << ")\n";

  indent_prefix = pfx;
}


void airl::dbg::enable_level_prefix(bool enabled)
{
  out(debug) << prefix(debug) << "airl::dbg::enable_level_prefix("
             << (enabled ? TRUE_STRING : FALSE_STRING) << ")\n";
...
void airl::dbg::enable_time_prefix(bool enabled)
{
  out(debug) << prefix(debug) << "airl::dbg::enable_time_prefix("
             << (enabled ? TRUE_STRING : FALSE_STRING) << ")\n";
...
std::ostream &airl::dbg::operator<<(std::ostream &s, const prefix &p)
{
...
std::ostream &airl::dbg::operator<<(std::ostream &s, const indent &i)
{
  s << indent_prefix.c_str();
  do_prefix(i.l, s);
...
std::ostream &airl::dbg::operator<<(std::ostream &s, const source_pos &pos)
{
  print_pos(s, pos);
  return s;
}
...
void airl::dbg::set_assertion_behaviour(level lvl, airl::dbg::assertion_behaviour b)
{
  out(debug) << prefix(debug) << "airl::dbg::set_assertion_behaviour("
...
void airl::dbg::set_assertion_period(dbgclock_t p)
{
  out(debug) << prefix(debug) << "airl::dbg::set_assertion_period("
             << p << ")\n";
  ...
  if (p && STDCLK::clock() == -1)
  {
    ...
    out(debug) << prefix(debug)
               ...
               << "airl::dbg::set_assertion_period is not supported.\n";
  }
}
...
void airl::dbg::assertion(airl::dbg::level lvl, airl::dbg::dbg_source src,
...
void airl::dbg::sentinel(airl::dbg::level lvl, airl::dbg::dbg_source src, const source_pos &here)
{
...
void airl::dbg::unimplemented(airl::dbg::level lvl, airl::dbg::dbg_source src,
...
void airl::dbg::check_ptr(airl::dbg::level lvl, airl::dbg::dbg_source src,
...
void airl::dbg::check_bounds(airl::dbg::level lvl, airl::dbg::dbg_source src,
...
airl::dbg::trace::trace(func_name_t name)
  : m_src(0), m_name(name), m_pos(AIRL_DBG_HERE), m_triggered(false)
{
  determine_source(m_src, m_pos);

  if (source_map[m_src].enabled(airl::dbg::tracing))
  {
    trace_begin();
  }
}
...
airl::dbg::trace::trace(dbg_source src, func_name_t name)
  : m_src(src), m_name(name), m_pos(AIRL_DBG_HERE), m_triggered(false)
{
  determine_source(m_src, m_pos);
...
airl::dbg::trace::trace(const source_pos &where)
...
airl::dbg::trace::trace(dbg_source src, const source_pos &where)
  : m_src(src), m_name(0), m_pos(where), m_triggered(false)
{
  determine_source(m_src, m_pos);

  if (source_map[src].enabled(airl::dbg::tracing))
  {
    trace_begin();
  }
}
...
void airl::dbg::trace::trace_begin()
{
  std::ostream &o = out(airl::dbg::tracing, m_src);
  o << indent(tracing);
  indent_depth++;
  o << TRACE_IN;
  if (m_name)
...
namespace
{
  ...
  source_info::source_info(ConstructionStyle cs)
    : levels(cs ? source_map[airl::dbg::default_source].levels : 0),
      dbg_streams(raw_cast().dbg_streams)
  {
    if (cs)
    {
      new (raw_dbg_streams)
        array_type(source_map[airl::dbg::default_source].raw_cast());
    }
    else
    {
      new (raw_dbg_streams) array_type;
      // add cerr to the error and fatal levels.
      add_ostream(airl::dbg::error, std::cerr);
      add_ostream(airl::dbg::fatal, std::cerr);
    }
  }
  ...
  void source_info::enable(airl::dbg::level lvl, bool status)
  {
  ...
  void source_info::add_ostream(airl::dbg::level lvl, std::ostream &o)
  {
    if (lvl == airl::dbg::all)
    {
  ...
  void source_info::remove_ostream(airl::dbg::level lvl, std::ostream &o)
  {
    if (lvl == airl::dbg::all)
    {
      for (unsigned int n = 0; n < NUM_DBG_LEVELS; ++n)
  ...
  void source_info::clear_ostream(airl::dbg::level lvl)
  {
    if (lvl == airl::dbg::all)
    {
      for (unsigned int n = 0; n < NUM_DBG_LEVELS; ++n)
  ...
  std::ostream &source_info::out(airl::dbg::level lvl)
  {
    if (lvl == airl::dbg::none || !enabled(lvl))
    {
      return null_ostream;
    }
    else
  ...
  void do_assertion_behaviour(airl::dbg::level lvl, constraint_type why,
                              const airl::dbg::source_pos &pos)
  {
    switch (lvl != airl::dbg::fatal ? behaviour[lvl] : airl::dbg::assertions_abort)
    {
      case airl::dbg::assertions_abort:
      {
        abort();
        break;
      }
      case airl::dbg::assertions_throw:
      {
        switch (why)
        {
          default:
          case why_assertion:
          {
            throw airl::dbg::assertion_exception(pos);
            break;
          }
          case why_sentinel:
          {
            throw airl::dbg::sentinel_exception(pos);
            break;
          }
          case why_unimplemented:
          {
            throw airl::dbg::unimplemented_exception(pos);
            break;
          }
          case why_check_ptr:
          {
            throw airl::dbg::check_ptr_exception(pos);
            break;
          }
        }
        break;
      }
      case airl::dbg::assertions_continue:
      default:
      {
        break;
      }
  ...
  void do_prefix(airl::dbg::level lvl, std::ostream &s)
  {
    ...
    if (level_prefix)
    {
      switch (lvl)
      {
        case airl::dbg::info:    { s << "   info: "; break; }
        case airl::dbg::warning: { s << "warning: "; break; }
        case airl::dbg::error:   { s << "  error: "; break; }
        case airl::dbg::fatal:   { s << "  fatal: "; break; }
        case airl::dbg::tracing: { s << "  trace: "; break; }
        case airl::dbg::debug:   { s << "  debug: "; break; }
        case airl::dbg::none:    {                   break; }
        case airl::dbg::all:     { s << "    all: "; break; }
      }
    }
  }
